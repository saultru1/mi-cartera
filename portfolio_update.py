#!/usr/bin/env python3
"""
Dashboard de cartera personal - Saúl
Descarga precios de Yahoo Finance y genera un HTML listo para abrir en el navegador.
...
# ─────────────────────────────────────────────
#  POSICIONES  (edita aquí tu cartera)
# ─────────────────────────────────────────────
POSITIONS = [
    # nombre,               ticker yfinance,  qty,  coste_eur,  divisa,    fecha_compra
    ("Adobe Inc",           "ADBE",           7,    1480.47,    "USD",     "2025-11-19"),
    ("QinetiQ Group PLC",   "QQ.L",           150,  711.27,     "GBX",     "2026-05-12"),
    ("Microsoft Corp",      "MSFT",           2,    638.99,     "USD",     "2026-03-26"),
    ("Greggs PLC",          "GRG.L",          16,   445.90,     "GBX",     "2025-03-04"),
    ("Wendy's Co",          "WEN",            37,   444.68,     "USD",     "2025-01-16"),
    ("Italian Sea Group",   "TISG.MI",        46,   418.41,     "EUR",     "2024-06-18"),
    ("PayPal Holdings",     "PYPL",           7,    351.94,     "USD",     "2026-01-16"),
    ("FactSet Research",    "FDS",            2,    349.79,     "USD",     "2026-02-05"),
    ("Novo Nordisk B",      "NOVO-B.CO",      5,    245.83,     "DKK",     "2025-07-29"),
    ("NICE Ltd ADR",        "NICE",           2,    235.02,     "USD",     "2025-09-02"),
    ("FRP Advisory",        "FRP.L",          150,  221.76,     "GBX",     "2025-06-17"),
    ("Nomad Foods",         "NOMD",           18,   218.49,     "USD",     "2025-09-10"),
    ("Evolution AB",        "EVO.ST",         3,    218.04,     "SEK",     "2024-12-23"),
    ("Dusk Group Ltd",      "DSK.AX",         5,    210.70,     "AUD",     "2023-05-29"),
    ("Euronet Worldwide",   "EEFT",           3,    205.82,     "USD",     "2025-10-28"),
    ("Enea SA",             "ENE.WA",         80,   205.32,     "PLN",     "2022-11-07"),
    ("Alphabet Inc A",      "GOOGL",          2,    205.23,     "USD",     "2022-09-23"),
    ("Novo Nordisk ADR",    "NVO",            6,    202.90,     "USD",     "2026-03-16"),
    ("Brown-Forman B",      "BF-B",           3,    91.59,      "USD",     "2025-02-07"),
    ("Power REIT",          "PW",             47,   10.77,      "USD",     "2022-12-27"),
]
...
COLORS = [
    ...
    "#6ee7b7","#fcd34d","#f9a8d4","#94a3b8","#cbd5e1",
]


def fetch_fx():
    """Descarga tipos de cambio respecto al EUR."""
    fx = {"EUR": 1.0, "GBX": None}
    print("  Descargando FX rates...", end=" ", flush=True)
    for cur, ticker in FX_TICKERS.items():
        try:
            t = yf.Ticker(ticker)
            rate = t.fast_info.get("lastPrice") or t.history(period="1d")["Close"].iloc[-1]
            # EURUSD=X = cuántos USD vale 1 EUR → 1 USD = 1/rate EUR
            fx[cur] = 1.0 / float(rate)
        except Exception as e:
            print(f"\n    ⚠ FX {cur} fallback: {e}")
    fx["GBX"] = fx.get("GBP", 0.01175) / 100   # pence = GBP/100
    print(f"OK  (1 USD = {fx.get('USD',0.92):.4f} EUR)")
    return fx


def fetch_price_and_history(ticker):
    """Devuelve precio actual, precio inicio de año y precio inicio de mes."""
    t = yf.Ticker(ticker)
    ...
    ytd_date = date(now.year, 1, 1)
    mtd_date = date(now.year, now.month, 1)

    def find_close_on_or_after(df, target):
        mask = df.index.date >= target
        subset = df[mask]
        return float(subset["Close"].iloc[0]) if not subset.empty else None

    ytd_price = find_close_on_or_after(hist, ytd_date)
    mtd_price = find_close_on_or_after(hist, mtd_date)

    return float(price), ytd_price, mtd_price
...
def main():
    ...
    for i, (name, ticker, qty, cost_eur, cur, buy_date) in enumerate(POSITIONS):
        print(f"  [{i+1:02d}/{len(POSITIONS)}] {name} ({ticker})...", end=" ", flush=True)
        try:
            price, ytd_p, mtd_p = fetch_price_and_history(ticker)
            if price is None:
            ...
            print(f"€{price_eur:.2f}  P&L: {'+' if pnl_pct>=0 else ''}{pnl_pct:.1f}%")
        except Exception as e:
            print(f"ERROR: {e}")
            pos_data.append({
    ...
    for bname, bticker in BENCHMARKS:
        try:
            price, ytd_p, mtd_p = fetch_price_and_history(bticker)
            ...
            print(f"    {bname}: ${price:.2f}  YTD: {ytd_ret:+.1f}%" if ytd_ret else f"    {bname}: ${price:.2f}")
        except Exception as e:
            print(f"    {bname}: ERROR {e}")
            bench_data.append({"name": bname, "ticker": bticker, "price": None, "ytd": None, "mtd": None})
    ...
    cagr  = ((1 + total_pnl_pct / 100) ** (1 / years) - 1) * 100 if years > 0 else 0

    # Weighted YTD & MTD
    ytd_num, ytd_den, mtd_num, mtd_den = 0, 0, 0, 0
    for p in valid:
        ...
        if p.get("ytd_ret") is not None and bd < ytd_start:
            ytd_num += p["ytd_ret"] * p["cost_eur"]
            ytd_den += p["cost_eur"]
        if p.get("mtd_ret") is not None and bd < mtd_start:
            mtd_num += p["mtd_ret"] * p["cost_eur"]
            mtd_den += p["cost_eur"]

    ytd_portfolio = ytd_num / ytd_den if ytd_den > 0 else None
    mtd_portfolio = mtd_num / mtd_den if mtd_den > 0 else None
    ...
    # 5. Generar HTML
    generate_html(pos_data, bench_data, portfolio, fx)
    print(f"\n✅ Dashboard generado: portfolio_dashboard.html")
    print(f"   Valor cartera:  €{total_val:,.0f}")
    print(f"   P&L total:      €{total_pnl:+,.0f} ({total_pnl_pct:+.1f}%)")
    print(f"   CAGR:           {cagr:+.1f}%")
    if ytd_portfolio: print(f"   YTD:            {ytd_portfolio:+.1f}%")
    if mtd_portfolio: print(f"   MTD:            {mtd_portfolio:+.1f}%")


def generate_html(pos_data, bench_data, portfolio, fx):
...
<style>
:root{{
  --bg:#0e1117;--bg2:#161b27;--bg3:#1e2535;--bg4:#252d40;
  --border:rgba(255,255,255,0.07);--text:#dde1ea;--muted:#6b7585;
  --blue:#4f8ef7;--green:#34d399;--red:#f87171;--amber:#fbbf24;--purple:#a78bfa;
...
.bar-val{{font-weight:600;font-family:var(--mono);}}
...
<div class="hdr">
  ...
  <div class="ts">
    ...
    <strong style="color:var(--text)">{portfolio['updated']}</strong><br><br>
    <small>Ejecuta portfolio_update.py<br>para actualizar</small>
  </div>
</div>
...
<div class="pos-card">
  <div class="pos-card-title">
    ...
    <span class="hint">Yahoo Finance · python portfolio_update.py para actualizar</span>
  </div>
  <table>
    <thead><tr>
...
<div class="footer">
  Datos: <strong>Yahoo Finance (yfinance)</strong> · Script: <strong>portfolio_update.py</strong> · Generado: {portfolio['updated']}
</div>
...
// Perf bars
const spYtd   = BENCH[0]?.ytd;
const msciYtd = BENCH[1]?.ytd;
const bars = [
...
g('benchGrid').innerHTML = [
  ...BENCH.map(b=>{{
    const ytdStr = b.ytd!==null ? fp(b.ytd) : '—';
    const mtdStr = b.mtd!==null ? fp(b.mtd) : '—';
    const ytdCol = b.ytd>=0 ? 'var(--green)' : 'var(--red)';
    const mtdCol = b.mtd>=0 ? 'var(--green)' : 'var(--red)';
    return `<div class="bench-item">
      <div class="bn">${{b.name}} (${{b.ticker}})</div>
      <div class="bv neu">${{b.price ? '$'+fmt(b.price,2) : '—'}}</div>
      <div class="bs">
        <span style="color:${{ytdCol}}">YTD: ${{ytdStr}}</span>
        <span style="color:${{mtdCol}}">MTD: ${{mtdStr}}</span>
      </div>
    </div>`;
  }}),
  `<div class="bench-item"><div class="bn">Total depositado</div><div class="bv neu">€${{fmt(PORT.deposited)}}</div><div class="bs"><span>24 aportaciones</span></div></div>`,
  `<div class="bench-item"><div class="bn">Comisiones DEGIRO</div><div class="bv neg">−€153</div><div class="bs"><span>históricas</span></div></div>`,
].join('');
...
g('posBody').innerHTML = sorted.map((p,i)=>{{
  ...
  return `<tr>
    ...
    out_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "portfolio_dashboard.html")
...
if __name__ == "__main__":
    try:
        ...
        print("❌ Falta instalar yfinance. Ejecuta:")
